...
export class AppService {
  ...
  addToFavorites(menuItem: MenuItem) {
    const url = 'http://localhost:8080/menu-items/favorites'; // Remplacez l'URL par l'endpoint approprié de votre API

    this.http.post(url, menuItem).subscribe(
      (response: any) => {
        console.log(response);
        // Effectuez des actions supplémentaires si nécessaire
      },
      (error: any) => {
        console.error('Une erreur s\'est produite lors de l\'ajout aux favoris', error);
      }
    );
  }

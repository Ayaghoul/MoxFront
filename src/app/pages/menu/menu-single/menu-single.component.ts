...
export class MenuSingleComponent implements OnInit {
    ...
    public getMenuItemById(id: number) {
        this.menuItemService.getMenuItemById(id).subscribe(data => {
    ...
    public addToCart() {
        ...
        if (this.menuItem.cartCount <= this.menuItem.availableCount) {
            ...
            this.snackBar.open('You can not add more items than available. In stock ' + this.menuItem.availableCount + ' items and you already added ' + this.menuItem.cartCount + ' item to your cart', '×', {
                panelClass: 'error',
